...
describe NotificationMailer, type: :mailer do
  ...
  let(:user) { FactoryGirl.create(:user, email: email, organization: organization_1) }
  let(:users) { [user] }
  let(:invites) { [FactoryGirl.create(:invite, :with_code, email: email, organization: organization_2, created_by: user)] }
